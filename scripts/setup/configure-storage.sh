...
LOCAL_SETTINGS_PATH="./UpdateEngine.Functions/src/local.settings.json"

if [[ "$STORAGE_MODE" == "FileSystem" ]]; then
    echo "📁 Configuring for local file system storage..."
    
    # Create directories
    mkdir -p "./UpdateEngine.Functions/src/store"
    mkdir -p "./UpdateEngine.Functions/src/content"
...
if [[ "$USE_APPHOST" == true ]]; then
    ...
    APPHOST_PATH="./UpdateEngine.AppHost/src/Program.cs"
